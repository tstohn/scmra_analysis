...
def run_permutation_analysis_with_fixed_differences(egfrInhDataProcessed, masterCnrResult):
    ...
    for i in range(PERMUTATIONS):
        ...
        #shuffle the labels...
        group_annot_shuffled = shuffle_group_label(group_annot)
...
def write_edge_subpopulation_differences(trueEdges, resultPermSimulations):
    ...
    for i in resultEdgePerm:
        ...
        for col in colName:
            for row in rowNames:
                if(not row=="UNTREATED"):
                    arr = [row, col, i.loc["UNTREATED", col] - i.loc[row, col]]
                    dfEdgeDiff = pd.concat([dfEdgeDiff, pd.DataFrame([arr], columns=["GROUP","EDGE","VALUE"])], axis=0)

    dfEdgeDiff.to_csv("./edgeSignificanceEGFRINH_RecoveredDifferences_2GROUPS.tsv", sep="\t")
    ...
    populations.remove("UNTREATED")
    for edge in colsToKeep:
        for pop in populations:
            toFromList = edge.split("_")
            val = trueEdges.rloc["UNTREATED"].loc[toFromList[0], toFromList[1]] - trueEdges.rloc[pop].loc[toFromList[0], toFromList[1]]
            arr = [pop, edge, val]
            truePoint = pd.concat([truePoint, pd.DataFrame([arr], columns=["GROUP","EDGE","VALUE"])], axis=0)

    truePoint.to_csv("./edgeSignificanceEGFRINH_TrueDifferences_2GROUPS.tsv", sep="\t")


egfrInhDataProcessed = pd.read_csv("./EGFRInhibitionDataProcessed_2GROUPS.tsv",sep='\t')

objectMasterResult = open("./MASTERCNR_EGFRINH_CNRRESULT_2GROUPS.pickle", "rb")
