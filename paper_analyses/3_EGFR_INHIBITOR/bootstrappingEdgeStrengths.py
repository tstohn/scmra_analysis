# Additional bootstrapping to fix found edges and check how the edge strength changes
...
def run_bootstrapping(egfrInhDataProcessed, masterCnrResult):
    ...
    #difference result frame
    cols = ["SIMULATION","GROUP","EDGE","VALUE"]
    results = pd.DataFrame(columns = cols)

    #actual value result frame
    valueCols = ["SIMULATION","EDGE","EGFRINH_VALUE","VEH_VALUE"]
    valResults = pd.DataFrame(columns = valueCols)
    ...
    for i in range(PERMUTATIONS):
        ...
        #fix the origionally found edges
        ...
        #get all edges that deviate in this simulation (should be the ones set before)
        deviatingFromToList = []
        ...
        tmpEdgeActualValueFrame = pd.DataFrame(columns = valueCols)
        ...
        for edge in deviatingFromToList:
            ...
            egfrInhValue = s.rloc["AG1478"].loc[edge[0],edge[1]]
            vehValue = s.rloc["vehicle"].loc[edge[0],edge[1]]
            line = [i, edgeName, egfrInhValue, vehValue]
            tmpEdgeActualValueFrame = pd.concat([tmpEdgeActualValueFrame, pd.DataFrame([line], columns=valueCols)], axis=0)

        results = pd.concat([results, dfEdgeDiff])
        valResults = pd.concat([valResults, tmpEdgeActualValueFrame])

    return(results, valResults)
...
def write_edge_subpopulation_differences(trueEdges, resultPermSimulations, resultsActualEdgeValues):

    resultPermSimulations.to_csv("../bin/bootstrap_FixedEdges_RecoveredStrengths_EGFR.tsv", sep="\t")

    resultsActualEdgeValues.to_csv("../bin/bootstrap_FixedEdges_ActualValues_EGFR.tsv", sep="\t")
    ...
    truePoint.to_csv("../bin/bootstrap_TrueDifferences_FixedEdges_EGFR.tsv", sep="\t")
...
resultPermSimulations, resultsActualEdgeValues = run_bootstrapping(egfrInhDataProcessed, masterCnrResult)
write_edge_subpopulation_differences(masterCnrResult, resultPermSimulations, resultsActualEdgeValues)
