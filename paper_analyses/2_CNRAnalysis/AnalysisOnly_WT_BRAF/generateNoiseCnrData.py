...
logFile = "../bin/logCnrNoiseDataGeneration.txt"
f = open(logFile, "w+")

for noise in noiseArray:
    for cell in cellNumArray:
        count = 0
        etaWasEstimated = False
        eta = None
        for repeat in range(repeats):
            ...
            #estimate eta only once for the same number of cells, noise
            if(not etaWasEstimated):                
                eta, etaListNew = estimate_eta_cnr(rglob, rtot, cell_annot, tx_annot, group_annot, withFlattening = True)
                etaWasEstimated = True
...
now = datetime.now()
folder = "../bin/"
name = "SIMULATION_CNR_NOISE_FIXETA"
